...
void listarEmpleadosXSector(eEmpleado x[],int tam, eSector sectores[], int tamSector)
{
    ...
    for(int i=0; i< tam; i++)
    {
        if(x[i].isEmpty == 0 && x[i].idSector == idSector) //que este ocupado y que el sector sea el mismo q se ingreso.
        {
...
void ordenarXSectorYNombre(eEmpleado x[],int tam, eSector sectores[], int tamSector)
{
    ...
    char descripcionI[20]; // auxiliares para descripcion del sector.
...
void mostrarEmpleadosMasGanadores(eEmpleado x[],int tam, eSector sectores[], int tamSector)
{
    ...
    for(int i=0; i < tamSector; i++)
    {
        ...
        for(int j=0; j < tam; j++)
        {
            if( (x[j].sueldo > maxSueldo && x[j].isEmpty == 0 && x[j].idSector == sectores[i].id) || flag == 0)//o el primero q ingresa es el max sueldo.
            {
